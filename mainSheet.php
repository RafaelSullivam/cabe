...
$options = new Options();
        // permite carregar CSS/imagens via URL
        $options->set('isRemoteEnabled', true);
        // define diretório temporário e cache de fontes
        $options->setTempDir('/var/www/html/ApiReacttmp');
        $options->setFontCache('/var/www/html/ApiReacttmp');
        // opcional: limite de acesso a este chroot
        $options->setChroot('/var/www/html/ApiReact');
          $dompdf = new Dompdf($options);
        
        // Limpar buffer de output se existir
        if (ob_get_level()) {
            ob_end_clean();
        }
    ...
    public function getData($codSample, $numberSample)
    {
        ...
        if ($dataHeaderAndFooter !== null) {
            $query = "SELECT * FROM CadastrodeClientes WHERE Processo = :Processo";
            $stmt = $pdo->prepare($query);
            $stmt->bindParam(':Processo', $dataHeaderAndFooter['Processo'], PDO::PARAM_INT);
            $stmt->execute();
        ...
        $query = "SELECT * FROM IndiceSuporteCalifornia WHERE Amostra = :codSample";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':codSample', $codSample, PDO::PARAM_INT);
        $stmt->execute();
        ...
        $stmt = $pdo->prepare($query);
        $codSample = (int)$codSample;
        $stmt->bindParam(':codSample', $codSample, PDO::PARAM_INT);
        $stmt->bindParam(':numberSample', $numberSample, PDO::PARAM_INT);
        $stmt->execute();
        $dataImagemLimitesAttemberg = $stmt->fetchAll(PDO::FETCH_ASSOC);
